...
class CausalSelfAttention(nn.Module):
    ...
    def apply_rotary_emb(self, xq: torch.Tensor, xk: torch.Tensor, T: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        device = xq.device
        seq_pos = torch.arange(T, device=xq.device, dtype=torch.float)   # Shape: (T)
        freqs = torch.einsum('i,j->ij', seq_pos, self.inv_freq.to(device)) # Shape: (T, dim // 2)
    ...
    def forward(self, x):
        ...
        qkv = self.c_attn(x)
        qkv = qkv.view(B, T, 3, self.n_head, C // self.n_head)
        q, k, v = qkv.unbind(2)
        ...
        for _ in range(max_new_tokens):
            ...
            if not do_sample:
                ...
                if top_p is not None:
                    ...
                    keep_m = cum_probs<=(1-top_p)
                    keep_m = torch.cat([keep_m[:, :1],keep_m[:,:-1]], dim=1)
                    og_keep_m = torch.zeros_like(probs, dtype=bool).scatter(1, sorted_indices, keep_m) 
                    scores_p = logits.masked_fill(~og_keep_m, -float('inf'))  
                    probs_p = torch.softmax(scores_p, dim=-1)     
                    idx_next = torch.multinomial(probs_p, 1)
